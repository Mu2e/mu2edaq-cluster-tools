...
if [[ "${1:-}" == "--uninstall" ]]; then
    ...
    # Remove launchers from any known bin dirs
    for bin_dir in "${HOME}/.local/bin" "${HOME}/bin"; do
        for cmd in "${APP_CMD}" lan-scan; do
...
info "Commands : ${BIN_DIR}/${APP_CMD}, ${BIN_DIR}/lan-scan"
...
success "Reference config copied to ${INSTALL_DIR}"
...
echo "Installing mu2edaq-cluster-tools package..."
...
exec "${INSTALL_DIR}/venv/bin/ssh-selector" "\$@"
LAUNCHER_EOF
chmod +x "${LAUNCHER}"
success "Launcher written to ${LAUNCHER}"

LAN_SCAN_LAUNCHER="${BIN_DIR}/lan-scan"
cat > "${LAN_SCAN_LAUNCHER}" <<LAUNCHER_EOF
#!/usr/bin/env bash
# Launcher for mu2edaq-cluster-tools / lan-scan
# Generated by install.sh — do not edit directly.
exec "${INSTALL_DIR}/venv/bin/lan-scan" "\$@"
LAUNCHER_EOF
chmod +x "${LAN_SCAN_LAUNCHER}"
success "Launcher written to ${LAN_SCAN_LAUNCHER}"
...
echo "Scan your LAN with:        lan-scan"
